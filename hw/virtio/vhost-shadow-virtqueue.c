...
static void vhost_svq_kick_packed(VhostShadowVirtqueue *svq)
{
    ...
    /*
     * We need to expose the available array entries before checking notification
     * suppressions.
     */
    smp_mb();
    ...
    if (virtio_vdev_has_feature(svq->vdev, VIRTIO_RING_F_EVENT_IDX)) {
        ...
        needs_kick = (svq->vring_packed.vring.device->flags != cpu_to_le16(VRING_PACKED_EVENT_FLAG_DISABLE));
    }
...
static bool vhost_svq_enable_notification(VhostShadowVirtqueue *svq)
{
    ...
    if (virtio_vdev_has_feature(svq->vdev, VIRTIO_RING_F_EVENT_IDX)) {
        ...
        if (svq->is_packed) {
            // svq->vring_packed.vring.driver->off_wrap = cpu_to_le16(svq->last_used_idx);
            svq->vring_packed.vring.driver->flags = cpu_to_le16(VRING_PACKED_EVENT_FLAG_ENABLE);
        } else {
...
static void vhost_svq_disable_notification(VhostShadowVirtqueue *svq)
{
    ...
    if (!virtio_vdev_has_feature(svq->vdev, VIRTIO_RING_F_EVENT_IDX)) {
        if (svq->is_packed) {
            svq->vring_packed.vring.driver->flags = cpu_to_le16(VRING_PACKED_EVENT_FLAG_DISABLE);
        } else {
            svq->vring.avail->flags |= cpu_to_le16(VRING_AVAIL_F_NO_INTERRUPT);
        }
    }
